...
for x in range(1, 11):#<---- as indexing images start from 1
	...
	im2 = im2[range(0,im.shape[0],2),:]   #<-removal of intermediate A-scans
	im = scipy.misc.imresize(im2,sz_L) #<-----  Interpolation to High resolution
	hh = np.array(Image.open(path1 + "HH" + str(x) + type)) 
	hh = scipy.misc.imresize(hh,sz_H) #<-----  Interpolation to High resolution
	if x==1:
