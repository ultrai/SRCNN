...
for I_Num = 1:18
    ...
    T2path = ('\1.tif');
    Test2path =  strcat(Testpath,T2path);
    LTest = double(imread(Test2path)); % load the nearby slice for the test image
    %  Test{2} = LTest;
    Test{2} = LTest(:,1:scale_factor:end);
    
    T3path = ('\2.tif');
    Test3path =  strcat(Testpath,T3path);
    LTest = double(imread(Test3path)); % load the nearby slice for the test image
    % Test{3} = LTest;
    Test{3} = LTest(:,1:scale_factor:end);
    
    T4path = ('\3.tif');
    Test4path =  strcat(Testpath,T4path);
    LTest = double(imread(Test4path)); % load the nearby slice for the test image
    % Test{4} = LTest;
    Test{4} = LTest(:,1:scale_factor:end);
    
    T5path = ('\4.tif');
    Test5path =  strcat(Testpath,T5path);
    LTest = double(imread(Test5path)); % load the nearby slice for the test image
    % Test{5} = LTest;
    Test{5} = LTest(:,1:scale_factor:end);
