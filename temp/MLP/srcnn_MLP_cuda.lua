...
def foo(dir):
    ...
    W=16
    window_shape = (W, W)
    for x in range(1, 2):#<---- as indexing images start from 1
        print(x)
...
cmd:option('-iterations',90,'total no of iterations')
cmd:text()
        ...
        for i = 1,inputs:size(1),opt.batch do
          ...
          count = count+1
            if i+opt.batch<inputs:size(1) then
                input = inputs[{{i,i+opt.batch},{}}]:cuda()
                target = targets[{{i,i+opt.batch},{}}]:cuda()
            else       
                input = inputs[{{i,inputs:size(1)},{}}]:cuda()
                target = targets[{{i,targets:size(1)},{}}]:cuda()
           end
