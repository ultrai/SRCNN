...
def foo(dir):
    ...
    for x in range(1, 11):#<---- as indexing images start from 1
        ...
        #im = im[:,range(0,im.shape[1],2)]   #<-removal of intermediate A-scans
        #im = scipy.misc.imresize(im,np.shape(im)) #<-----  Interpolation to High resolution
...
cmd:option('-h1',2000,'Number of filters to be considered')
cmd:option('-h2',2000,'Number of filters to be considered')
cmd:option('-drop',0.2,'Dropout')
cmd:option('-batch',2e5,'Number of filters to be considered')
cmd:option('-feat_sz',13,'Each filter size')
cmd:option('-feat_sz2',13,'Each filter size')
cmd:option('-iterations',200,'total no of iterations')
cmd:text()
...
optimMethod = optim.adam--cg--adam --rmsprop --lbfgs
scale = 1
for epoch =1,10 do
for i = 1,inputs:size(1),opt.batch do
       if i+opt.batch*scale<inputs:size(1) then
           input = inputs[{{i,i+opt.batch*scale},{}}]:float()
           target = targets[{{i,i+opt.batch*scalelyt},{}}]:float()
       else       
           input = inputs[{{i,inputs:size(1)},{}}]:float()
           target = targets[{{i,inputs:size(1)},{}}]:float()
       end
